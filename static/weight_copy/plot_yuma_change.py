...
def simulate_yuma_run(W, S, invalid_proof_tempo):
    ...
    run_dividends = {VALID_PROOF_VALUE: {"copier": [], "validators": []}}
    run_vtrust = {VALID_PROOF_VALUE: {"copier": [], "validators": []}}
    for tempo in range(1, NUM_RUNS + 1):
        ...
        pre_calculated_weights = (
            pre_result["server_consensus_weight"]
            / pre_result["server_consensus_weight"].sum()
        )
        ...
        result = utils.Yuma2(
            W_varied,
            S,
            valid_proof_value=VALID_PROOF_VALUE if tempo > invalid_proof_tempo else 0,
        )

        copier_reward = result["validator_reward_normalized"][-1].item()
        validator_rewards = result["validator_reward_normalized"][:-1].tolist()
        run_dividends[VALID_PROOF_VALUE]["copier"].append(copier_reward)
        run_dividends[VALID_PROOF_VALUE]["validators"].append(validator_rewards)

        copier_vtrust = result["validator_trust"][-1].item()
        validator_vtrusts = result["validator_trust"][:-1].tolist()
        run_vtrust[VALID_PROOF_VALUE]["copier"].append(copier_vtrust)
        run_vtrust[VALID_PROOF_VALUE]["validators"].append(validator_vtrusts)
...
def simulate_yuma_runs(W, S, invalid_proof_tempo):
    with concurrent.futures.ProcessPoolExecutor() as executor:
        futures = [
            executor.submit(simulate_yuma_run, W, S, invalid_proof_tempo)
            for _ in range(1)
        ]
        results = [
...
def plot_simulation(ax1, ax2, ax3, dividends, vtrust, num_runs, invalid_proof_tempo):
    ...
    ax1.plot(
        x,
        dividends[VALID_PROOF_VALUE]["copier"],
        label=f"Unproven Validator (Valid Proof Value={VALID_PROOF_VALUE})",
        color="red",
        linewidth=2,
    )
    validator_dividends = np.array(dividends[VALID_PROOF_VALUE]["validators"])
    median_dividends = np.median(validator_dividends, axis=1)
    std_dividends = np.std(validator_dividends, axis=1)
    ax1.plot(
        x, median_dividends, label="Median Proven Validator", color="blue", linewidth=2
    )
    ax1.fill_between(
        x,
        median_dividends - std_dividends,
        median_dividends + std_dividends,
        color="blue",
        alpha=0.1,
    )
    ...
    ax2.plot(
        x,
        vtrust[VALID_PROOF_VALUE]["copier"],
        label=f"Unproven Validator (Valid Proof Value={VALID_PROOF_VALUE})",
        color="red",
        linewidth=2,
    )
    validator_vtrust = np.array(vtrust[VALID_PROOF_VALUE]["validators"])
    median_vtrust = np.median(validator_vtrust, axis=1)
    std_vtrust = np.std(validator_vtrust, axis=1)
    ax2.plot(
        x, median_vtrust, label="Median Proven Validator", color="blue", linewidth=2
    )
    ax2.fill_between(
        x,
        median_vtrust - std_vtrust,
        median_vtrust + std_vtrust,
        color="blue",
        alpha=0.1,
    )
    ...
    median_validator_dividends = np.median(
        dividends[VALID_PROOF_VALUE]["validators"], axis=1
    )
    relative_copier_dividends = (
        np.array(dividends[VALID_PROOF_VALUE]["copier"]) / median_validator_dividends
    )
    ...
    ax3.axhline(
        y=1, color="blue", linestyle="--", linewidth=1, label="Median Proven Validator"
    )
    ax3.axhline(
        y=0.82,
        color="orange",
        linestyle="--",
        linewidth=2,
        label="Minimum Profitability Threshold",
    )
    ...
    for ax in (ax1, ax2, ax3):
        ...
        ax.text(
            invalid_proof_tempo + 0.2,
            ax.get_ylim()[1] * 0.95,
            "Unproven Validator\nSubmits Invalid Proofs",
            ha="left",
            va="top",
            rotation=0,
            fontsize=10,
        ...
        ax.axvline(
            ...
            linestyle="--",
            linewidth=2,
            label="Consensus Copying Starts",
        )
        ax.text(
            COPY_START_TEMPO + 0.2,
            ax.get_ylim()[1] * 0.95,
            "Unproven Validator\nStarts Copying",
            ha="left",
            va="top",
